#!/usr/bin/env bash
ZK_USER=${ZK_USER:-"appuser"}
ZK_LOG_LEVEL=${ZK_LOG_LEVEL:-"INFO"}
ZK_DATA_DIR=${ZK_DATA_DIR:-"/home/appuser/zookeeper/data/data"}
ZK_DATA_LOG_DIR=${ZK_DATA_LOG_DIR:-"/home/appuser/zookeeper/data/datalog"}
ZK_LOG_DIR=${ZK_LOG_DIR:-"/home/appuser/zookeeper/logs"}
ZK_CONF_DIR=${ZK_CONF_DIR:-"/home/appuser/zookeeper/conf"}
...
function validate_env() {
    ...
    MY_POD=$(echo $POD_NAME | awk -F'-' '{print $NF}')
    MY_ID=$((MY_POD+1))
    ...
    do
        echo "server - $server"
	echo "HOST - $HOST"
        ZK_ENSEMBLE_LIST+="$server:$ZK_SERVER_PORT:$ZK_ELECTION_PORT "
        ID=$((ID+1))
    done
    NUM_SERVERS=$((ID-1))
    if [ -z $MY_ID ]; then
...
function create_data_dirs() {
    if [ ! -d $ZK_DATA_DIR  ]; then
        mkdir -p $ZK_DATA_DIR
        sudo chown -R $ZK_USER:$ZK_USER $ZK_DATA_DIR
    ...
    if [ ! -d $ZK_DATA_LOG_DIR  ]; then
        ...
        sudo chown -R $ZK_USER:$ZK_USER $ZK_DATA_LOG_DIR
    ...
    if [ ! -d $ZK_LOG_DIR  ]; then
        mkdir -p $ZK_LOG_DIR
        sudo chown -R $ZK_USER:$ZK_USER $ZK_LOG_DIR
        echo "Created ZooKeeper Log Directory"
        ls -ld $ZK_LOG_DIR >& 1
    fi

    echo "Creating ZooKeeper ensemble id file $ID_FILE"
    rm -f $ID_FILE
    if [ ! -f $ID_FILE ]; then
        echo $MY_ID >> $ID_FILE
    fi
